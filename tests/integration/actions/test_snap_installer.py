...
def test_inject_from_host(core20_lxd_instance, installed_snap):
    ...
    with installed_snap("hello-world"):
        snap_installer.inject_from_host(
            executor=core20_lxd_instance, snap_name="hello-world"
        )

    core20_lxd_instance.execute_run(["test", "-d", "/snap/hello-world"], check=True)


def test_inject_from_host_using_pack_fallback(core20_lxd_instance, empty_test_snap):
    snap_installer.inject_from_host(
...
def test_install_from_store_strict(core20_lxd_instance, installed_snap):
    core20_lxd_instance.execute_run(
        ["test", "!", "-d", "/snap/hello-world"], check=True
    )
...
def test_install_from_store_classic(core20_lxd_instance, installed_snap):
...
def test_install_from_store_channel(core20_lxd_instance, installed_snap):
