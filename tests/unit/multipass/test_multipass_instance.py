...
@pytest.fixture
def mock_multipass():
    m = mock.Mock(spec=Multipass)
    m.info.return_value = EXAMPLE_INFO
    yield m
...
def mock_os_getgid():
    with mock.patch("os.getgid", return_value=1234) as m:
        yield m


@pytest.fixture(autouse=True)
def mock_os_getuid():
    with mock.patch("os.getuid", return_value=4567) as m:
        yield m
...
def test_get_info(mock_multipass, instance):
    assert instance.get_info() == EXAMPLE_INFO["info"]["test-instance"]

    assert mock_multipass.mock_calls == [mock.call.info(instance_name="test-instance")]


def test_get_info_none(mock_multipass, instance):
    mock_multipass.info.return_value = None
    assert instance.get_info() is None
...
def test_is_mounted_vm_deleted(mock_multipass, instance):
    with pytest.raises(MultipassError) as exc_info:
        MultipassInstance(name="non-existent", multipass=mock_multipass).is_mounted(
            host_source=pathlib.Path("/home/user/not-mounted"),
            target=pathlib.Path("/root/project"),
        )

    assert exc_info.value == MultipassError(brief="VM 'non-existent' was deleted.")
...
def test_mount(mock_multipass, monkeypatch, platform):
    ...
    assert multipass_instance._get_host_gid() == 1234
    assert multipass_instance._get_host_uid() == 4567
...
def test_mount_win32(mock_multipass, monkeypatch):
    ...
    assert multipass_instance._get_host_gid() == 0
    assert multipass_instance._get_host_uid() == 0
...
@mock.patch("craft_providers.actions.linux.is_target_file", return_value=True)
def test_pull_file(mock_is_target_file, mock_multipass, instance, tmp_path):
...
@mock.patch("craft_providers.actions.linux.is_target_file", return_value=False)
def test_pull_file_no_source(mock_is_target_file, mock_multipass, instance, tmp_path):
    source = pathlib.Path("/tmp/src.txt")
    destination = tmp_path / "dst.txt"
...
@mock.patch("craft_providers.actions.linux.is_target_file", return_value=True)
def test_pull_file_no_parent_directory(
    mock_is_target_file, mock_multipass, instance, tmp_path
):
    source = pathlib.Path("/tmp/src.txt")
...
@mock.patch("craft_providers.actions.linux.is_target_directory", return_value=True)
def test_push_file(mock_is_target_directory, mock_multipass, instance, tmp_path):
    ...
    assert mock_multipass.mock_calls == [
        mock.call.transfer(source=str(source), destination="test-instance:/tmp/dst.txt")
    ]


@mock.patch("craft_providers.actions.linux.is_target_directory", return_value=False)
def test_push_file_no_source(
    mock_is_target_directory, mock_multipass, instance, tmp_path
):
    source = tmp_path / "src.txt"
    destination = pathlib.Path("/tmp/dst.txt")
...
@mock.patch("craft_providers.actions.linux.is_target_directory", return_value=False)
def test_push_file_no_parent_directory(
    mock_is_target_directory, mock_multipass, instance, tmp_path
):
    source = tmp_path / "src.txt"
    source.write_text("this is a test")
...
def test_supports_mount(mock_multipass, instance):
